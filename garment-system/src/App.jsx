...
import { 
  ...
  Save, Printer, Cloud, LogIn, LogOut, RefreshCw
} from 'lucide-react';
...
const App = () => {
  ...
  const uploadToCloud = async () => {
    ...
    const dataToSave = { employees, styles, workLogs, advances, lastUpdated: new Date().toISOString() };
    try {
  ...
  const downloadFromCloud = async () => {
    if(!user) return alert("Please login first!");
    if(!confirm("⚠️ Warning: Local data will be overwritten by Cloud data. Continue?")) return;
    try {
  ...
  // --- STYLE ACTIONS ---
  const addNewStyle = () => { 
    const cleanName = newStyleName.trim();
    if (!cleanName) return; 

    const exists = styles.some(s => s.name.toLowerCase() === cleanName.toLowerCase());
    if (exists) {
      alert("⚠️ Warning: Ye Style Name pehle se exist karta hai!");
      return;
    }

    setStyles([...styles, { id: Date.now(), name: cleanName, rates: [] }]); 
    setNewStyleName(''); 
  };
  ...
  const addNewRateToStyle = (styleId) => {
    if (!newRateName || newRatePrice === '') {
        alert("Please enter a Name.");
        return;
    }
    const currentStyle = styles.find(s => s.id === styleId);
    const rateExists = currentStyle.rates.some(r => r.name.toLowerCase() === newRateName.toLowerCase());
    if (rateExists) {
      alert("⚠️ Warning: Ye Rate/Operation is style me pehle se hai!");
      return;
    }
    setStyles(styles.map(s => s.id === styleId ? { ...s, rates: [...s.rates, { name: newRateName, rate: parseFloat(newRatePrice) }] } : s));
    setNewRateName(''); 
    setNewRatePrice(''); 
    if(rateNameInputRef.current) rateNameInputRef.current.focus();
  };
          ...
          <div className="space-y-6">
            <GlassCard>
              <div className="flex gap-4 justify-center">
                <GlassInput 
                  ...
                  value={newStyleName} 
                  onChange={handleTextChange(setNewStyleName)} 
                  onKeyDown={(e) => handleEnterKey(e, addNewStyle)}
                  className="max-w-md" 
                />
            ...
            <div className="grid gap-6">{styles.map(style => (<GlassCard key={style.id} className="relative overflow-hidden"><div className="flex justify-between items-start mb-4 border-b border-white/10 pb-4"><h3 className="font-bold text-xl text-indigo-300">{style.name}</h3><button onClick={() => deleteStyle(style.id)} className="text-red-400 text-xs hover:bg-red-500/20 px-2 py-1 rounded transition">Delete Style</button></div><div className="grid grid-cols-1 md:grid-cols-2 gap-3">{style.rates.map((r, idx) => (<div key={idx} className="bg-black/20 p-3 rounded-lg flex justify-between items-center group"><span className="text-gray-300 text-sm">{r.name}</span><div className="flex items-center gap-3">{editingRate?.styleId === style.id && editingRate?.rateIndex === idx ? (<div className="flex items-center gap-1"><input type="number" value={tempRateVal} onChange={(e) => setTempRateVal(e.target.value)} onKeyDown={(e) => handleEnterKey(e, () => updateRateValue(style.id, idx))} className="w-20 bg-black/40 border border-blue-500 rounded px-1 py-0.5 text-right text-sm text-white" autoFocus /><button onClick={() => updateRateValue(style.id, idx)} className="text-green-400"><Check size={16}/></button><button onClick={() => setEditingRate(null)} className="text-gray-400"><X size={16}/></button></div>) : (<><span className="font-semibold text-green-400">₹{r.rate.toFixed(2)}</span><div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity"><button onClick={() => { setEditingRate({ styleId: style.id, rateIndex: idx }); setTempRateVal(r.rate); }} className="text-blue-400 hover:text-blue-300"><Edit2 size={14} /></button><button onClick={() => deleteRateFromStyle(style.id, idx)} className="text-red-400 hover:text-red-300"><Trash2 size={14} /></button></div></>)}</div></div>))}{activeStyleForAdd === style.id ? (
              <div className="bg-blue-900/20 border border-blue-500/30 p-2 rounded-lg flex gap-2 items-center">
                <GlassInput 
                  ref={rateNameInputRef}
                  placeholder="Name (Required)" 
                  className="w-full bg-transparent border-b border-white/20 text-sm text-white focus:outline-none" 
                  value={newRateName} 
                  onChange={handleTextChange(setNewRateName)} 
                  onKeyDown={(e) => { 
                    if(e.key === 'Enter') {
                        e.preventDefault();
                        if(ratePriceInputRef.current) ratePriceInputRef.current.focus();
                    }
                  }}
                  autoFocus
                />
                <GlassInput 
                  ref={ratePriceInputRef}
                  placeholder="Rate (0 Allowed)" 
                  type="number" 
                  className="w-24 bg-transparent border-b border-white/20 text-sm text-white focus:outline-none" 
                  value={newRatePrice} 
                  onChange={e => setNewRatePrice(e.target.value)} 
                  onKeyDown={(e) => handleEnterKey(e, () => addNewRateToStyle(style.id))}
                />
            ...
            ) : (<button onClick={() => setActiveStyleForAdd(style.id)} className="border-2 border-dashed border-white/10 text-gray-400 rounded-lg p-2 text-sm hover:border-white/30 hover:text-white transition flex justify-center items-center gap-1"><Plus size={16} /> Add Rate</button>)}</div></GlassCard>))}</div>
          </div>
        )}
            ...
            {activeTab === 'employees' && (
              <GlassCard>
                <div className="flex gap-4 mb-6 justify-center">
                  <GlassInput 
                    value={newEmpName}
                    onChange={handleTextChange(setNewEmpName)} 
                    onKeyDown={(e) => handleEnterKey(e, addEmployee)}
                    placeholder="New Employee Name" 
                    className="max-w-md" 
                  />
                  <button onClick={addEmployee} className="bg-green-600 hover:bg-green-500 text-white px-6 rounded-lg">Add</button>
                </div>
                ...
                <GlassCard>
                  ...
                  <div className="space-y-4">
                    ...
                    <GlassInput 
                      ...
                      onKeyDown={(e) => handleEnterKey(e, addAdvance)} 
                    />
        ...
        {activeTab === 'settings' && (
          <div>
            <GlassCard className="text-center">
              <h2 className="text-2xl font-bold text-gray-200 mb-6">Cloud & Data Management</h2>

              {/* CLOUD SECTION */}
              <div className="mb-8 bg-indigo-900/30 border border-indigo-500/30 p-6 rounded-xl">
                <Cloud size={48} className="mx-auto text-indigo-400 mb-2" />
                <h3 className="text-xl font-semibold text-indigo-200 mb-4">Google Cloud Sync</h3>
                
                {!user ? (
                  <button onClick={handleGoogleLogin} className="bg-white text-gray-900 px-6 py-3 rounded-lg font-bold flex items-center justify-center gap-2 mx-auto hover:bg-gray-100 transition">
                    <LogIn size={20} /> Login with Google
                  </button>
                ) : (
                  <div className="space-y-4">
                    <p className="text-indigo-300">Logged in as: <span className="font-bold text-white">{user.displayName}</span></p>
                    <div className="flex gap-4 justify-center flex-wrap">
                      <button onClick={uploadToCloud} className="bg-indigo-600 hover:bg-indigo-500 text-white px-5 py-2 rounded-lg flex items-center gap-2">
                        <Upload size={18} /> Upload to Cloud
                      </button>
                      <button onClick={downloadFromCloud} className="bg-cyan-600 hover:bg-cyan-500 text-white px-5 py-2 rounded-lg flex items-center gap-2">
                        <Download size={18} /> Download from Cloud
                      </button>
                    </div>
                    <button onClick={handleLogout} className="text-red-400 text-sm hover:underline mt-2 flex items-center justify-center gap-1 mx-auto">
              ...
              {/* LOCAL FILE BACKUP */}
              <h3 className="text-lg font-semibold text-gray-400 mb-4">Local File Backup</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-blue-600/10 border border-blue-500/30 p-4 rounded-xl hover:bg-blue-600/20 transition">
                  <h3 className="font-semibold text-blue-200 mb-2">Save to File</h3>
                  <button onClick={exportData} className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg w-full flex items-center justify-center gap-2"><Save size={16} /> Export JSON</button>
                </div>
                <div className="bg-green-600/10 border border-green-500/30 p-4 rounded-xl hover:bg-green-600/20 transition">
                  <h3 className="font-semibold text-green-200 mb-2">Load from File</h3>
                  <label className="bg-green-600 hover:bg-green-500 text-white px-4 py-2 rounded-lg w-full flex items-center justify-center gap-2 cursor-pointer transition"><Upload size={16} /> Select JSON<input type="file" accept=".json" onChange={importData} className="hidden" /></label>
                </div>
              </div>
              ...
              <div className="mt-8 p-4 bg-red-500/10 border border-red-500/20 rounded-lg text-left">
                ...
                <p className="text-sm text-gray-400 mb-3">Clear all data and start fresh.</p>
                <button onClick={() => { if(confirm("DANGER: DELETE ALL DATA?")) { localStorage.clear(); window.location.reload(); }}} className="text-red-400 text-sm hover:underline">Reset Everything</button>
              </div>
            </GlassCard>
          </div>
        )}
