...
const ChatRoom = () => {
  ...
  // Activity States
  const [othersActivity, setOthersActivity] = useState(null); // "Priya is typing..."
  ...
  // Main Effect: Presence, Activity, Messages
  useEffect(() => {
    ...
    // 2. ACTIVITY CLEANUP (If I disconnect, clear my 'typing' status)
    const myActivityRef = ref(rtdb, `rooms/${roomId}/activity/${authName}`);
    onDisconnect(myActivityRef).remove();
    ...
    // 4. LISTEN TO OTHERS' ACTIVITY (Typing/Recording)
    const activityRef = ref(rtdb, `rooms/${roomId}/activity`);
    const unsubActivity = onValue(activityRef, (snapshot) => {
      const data = snapshot.val();
      if (data) {
        // Find first user who is NOT me
        const activeUserKey = Object.keys(data).find(key => key !== authName);
        if (activeUserKey) {
          const activity = data[activeUserKey];
          // activity.action contains 'typing', 'recording', 'uploading'
          if (activity.action === 'typing') setOthersActivity(`${activeUserKey} is typing...`);
          else if (activity.action === 'recording') setOthersActivity(`${activeUserKey} is recording audio...`);
          else if (activity.action === 'uploading') setOthersActivity(`${activeUserKey} is sending a file...`);
          else setOthersActivity(null);
        } else {
          setOthersActivity(null);
        }
      } else {
        setOthersActivity(null);
      }
    ...
    return () => { 
      ...
      remove(myActivityRef); // Clear activity on unmount
      unsubUsers(); 
      unsubActivity();
      unsubMessages(); 
    };
  ...
  // --- ACTIVITY HELPERS ---
  const setMyActivity = (action) => {
    // action: 'typing' | 'recording' | 'uploading' | null
    const refPath = ref(rtdb, `rooms/${roomId}/activity/${authName}`);
    if (action) set(refPath, { action });
    else remove(refPath);
  };
  ...
  const handleTyping = (e) => {
    ...
    // Only set typing if not already recording/uploading
    if (!isRecording && !uploading) {
        setMyActivity('typing');
        
        // Debounce: Clear typing status after 2 seconds of inactivity
  ...
  const handleKeyDown = (e) => { 
    if (e.key === 'Enter' && !e.shiftKey) { 
      e.preventDefault(); 
      setMyActivity(null); // Stop typing status immediately on send
      editingMsgId ? submitEdit() : sendMessage(e); 
    }
  };
  ...
  const sendMessage = async (e) => {
    ...
    setMyActivity(null); // Clear typing
  ...
  const handleFileUpload = async (e) => {
    ...
    setUploading(true);
    setMyActivity('uploading'); // START UPLOADING STATUS
    try {
    ...
    setUploading(false);
    setMyActivity(null); // STOP STATUS
  };
  ...
  const startRecording = async () => {
    try {
      ...
      recorder.onstop = async () => {
        ...
        try { 
            const url = await uploadToCloudinary(file); 
            await addDoc(collection(db, `rooms/${roomId}/messages`), { text: "Voice Note", mediaUrl: url, type: 'audio', sender: authName, createdAt: serverTimestamp(), uid: uuidv4() }); 
        } catch (err) {}
        setUploading(false); setMyActivity(null); // Clear status
        stream.getTracks().forEach(t => t.stop());
      };
      recorder.start(); 
      setMediaRecorder(recorder); 
      setIsRecording(true);
      setMyActivity('recording'); // START RECORDING STATUS
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden relative select-none text-sm md:text-base">
      ...
      <div className="px-6 shrink-0 z-10 h-6">
        {othersActivity && (
           <div className="flex items-center gap-2 animate-pulse">
              <span className="w-1.5 h-1.5 bg-blue-400 rounded-full"></span>
              <p className="text-[10px] text-blue-200 font-medium tracking-wide">{othersActivity}</p>
           </div>
        )}
      </div>
      ...
      <div className="p-4 shrink-0 z-10">
        ...
        <div className={`glass p-2 pl-2 flex items-center gap-2 ${editingMsgId ? 'rounded-b-2xl rounded-t-none' : 'rounded-full'}`}>
          ...
          <input 
            type="text" 
            value={newMessage} 
            onChange={handleTyping} // <-- Changed to handleTyping
            onKeyDown={handleKeyDown} 
            placeholder={isRecording ? "Recording..." : (editingMsgId ? "Update..." : "Message...")} 
            disabled={isRecording} 
            className="flex-1 bg-transparent text-white placeholder-gray-400 outline-none min-w-0" 
          />
          
          {!editingMsgId && <button onClick={isRecording ? stopRecording : startRecording} className={`p-2 rounded-full transition-all ${isRecording ? 'bg-red-500 animate-pulse text-white' : 'hover:bg-white/10 text-gray-300'}`}>
             {isRecording ? <div className="flex items-center gap-1"><StopCircle className="w-5 h-5" /> <MoreHorizontal className="w-4 h-4 animate-bounce" /></div> : <Mic className="w-5 h-5" />}
          </button>}
