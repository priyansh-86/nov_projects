...
function App() {
  ...
  // --- EFFECTS ---
  useEffect(() => {
    document.title = image ? `Editing: ${meta.name}` : "PRIYANSH | Image View";
  }, [image, meta.name]);
    ...
    <div className="relative w-full h-screen bg-[#050505] text-white overflow-hidden font-sans selection:bg-blue-500/30">
      ...
      <AnimatePresence>
        {image && (
          ...
          >
            {/* The Blurred Glow Image */}
            <img 
              src={image} 
              alt="ambient" 
              className="w-full h-full object-cover blur-[120px] scale-150 opacity-25 saturate-150" 
            />
      ...
      <nav className="absolute top-0 left-0 w-full p-6 flex justify-between items-center z-50 pointer-events-none">
        ...
        <div className="flex items-center gap-3 pointer-events-auto">
          ...
          <span className="font-bold tracking-tight text-lg uppercase hidden sm:block text-white/90 drop-shadow-lg">
            PRIYANSH | Image View
          </span>
          <span className="font-bold tracking-tight text-lg uppercase sm:hidden text-white/90 drop-shadow-lg">
            PRIYANSH
          </span>
          
          {image && (
            <div className="flex items-center gap-2 ml-4 px-3 py-1 bg-black/20 backdrop-blur-md border border-white/5 rounded-full text-xs text-white/70 shadow-lg transition-all hover:bg-black/40">
               <span>{meta.dimensions}</span>
               <span className="hidden sm:inline w-1 h-1 bg-white/20 rounded-full"></span>
               <span className="hidden sm:inline">{meta.size}</span>
            </div>
          )}
        </div>
        ...
        <div className="pointer-events-auto flex items-center gap-3">
           ...
           <button 
             ...
             className="flex items-center gap-2 px-4 py-2 bg-black/20 hover:bg-white/10 border border-white/5 rounded-full text-sm font-medium transition-all hover:scale-105 backdrop-blur-md shadow-lg group"
           >
             <UserCircle2 size={18} className="text-white/80 group-hover:text-white" />
             <span className="hidden sm:inline text-white/90 group-hover:text-white">Connect</span>
           </button>
           ...
           <AnimatePresence>
            {image && (
                ...
                onClick={handleClose} 
                className="bg-black/20 hover:bg-red-500/20 hover:text-red-400 border border-white/5 p-2 rounded-full backdrop-blur-md transition-all shadow-lg"
                >
                <X size={20} />
                </motion.button>
            )}
